...
# #         # mapping_efi.json se trouve 1 niveau au-dessus de ce fichier (api/mapping_efi.json)
...
# #                 "delta_pct": delta_pct,
# #                 "efi_refs": efi_refs,
# #                 "significatif": (abs(delta) >= perf_mat) if perf_mat else False
# #             })
...
# #         # 2) Identité par compte  3) Cohérence du signe
# #         for x in lines:
...
# # Tolérance (en unité de monnaie). Modifiable via variable d'env BALANCE_TOLERANCE
# TOLERANCE = int(os.getenv("BALANCE_TOLERANCE", "1"))
...
#         # 1) Équilibre global avec tolérance
#         sum_deb_fin = sum(int(x.get("debit_fin", 0) or 0) for x in lines)
#         sum_cre_fin = sum(int(x.get("credit_fin", 0) or 0) for x in lines)
#         ecart_global = sum_deb_fin - sum_cre_fin
#         if abs(ecart_global) > TOLERANCE:
#             report["equilibre_global"] = False
#             report["erreurs"].append({
#                 "type": "equilibre_global",
#                 "message": f"Total débit fin ({sum_deb_fin}) − total crédit fin ({sum_cre_fin}) = {ecart_global} (> tolérance {TOLERANCE})"
#             })

#         # ➕ Récap totaux pour l’UI
#         report["totaux"] = {
...
#         # 2) Identité par compte & 3) Cohérence du signe (avec tolérance)
#         for x in lines:
#             di = int(x.get("debit_initial", 0) or 0)
#             ci = int(x.get("credit_initial", 0) or 0)
#             dm = int(x.get("debit_mvt", 0) or 0)
...
#             lhs = df - cf
#             rhs = (di - ci) + (dm - cm)
#             ecart_identite = lhs - rhs
#             if abs(ecart_identite) > TOLERANCE:
#                 report["erreurs"].append({
#                     "type": "identite_compte",
#                     "numero_compte": x.get("numero_compte"),
#                     "message": f"(df−cf)={lhs} − (di−ci+dm−cm)={rhs} = {ecart_identite} (> tolérance {TOLERANCE})"
#                 })

#             solde_reel = int(x.get("solde_reel", 0) or 0)
#             sign = x.get("sign_solde")
#             # si le solde est quasi nul, on ne déclenche pas d’erreur de signe
#             if abs(solde_reel) > TOLERANCE:
#                 expect_sign = "D" if solde_reel >= 0 else "C"
#                 if sign not in ("D", "C") or sign != expect_sign:
#                     report["erreurs"].append({
#                         "type": "signe_incoherent",
#                         "numero_compte": x.get("numero_compte"),
#                         "message": f"solde_reel={solde_reel} / sign_solde={sign} (attendu: {expect_sign}, tolérance {TOLERANCE})"
#                     })

#         # MAJ du nb d’erreurs après la boucle
#         report["totaux"]["nb_erreurs"] = len(report["erreurs"])
#         return report
...
class Mission(Document):
    ...
    def revue_analytique(self, id_mission):
        ...
        for row in variations:
            ...
            commentaire = (
                f"COMMENTAIRE: {', '.join(efi_refs) if efi_refs else 'Aucun'} — "
                f"COMMENTAIRE: {'Oui' if (perf_mat and abs(delta) >= perf_mat) else 'Non'}"
            )
